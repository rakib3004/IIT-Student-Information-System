...
float sub_cgpa[6];
int sub_marks;
float sem_cgpa[8]={0.0000,0.0000,0.0000,0.0000,0.0000,0.0000,0.0000,0.0000};
int search();
 int search1();
 int search2();
 int search3();
...
int profile()
{
    ...
    fprintf(ptr,"Name\tAddress\t\tSt_Roll_No\tRegistration\n");
    fprintf(ptr,"====\t=======\t\t==========\t============\n");
...
FILE *quality;
  ...
  printf("Enter the semester no:");
      ...
      fclose(quality);
        ...
        quality=fopen("Student_marks_database.txt","a+");
        current_cgpa[rt]=0;
        current_cgpa[rt]=(sem_cgpa[0]+sem_cgpa[1]+sem_cgpa[2]+sem_cgpa[3]+sem_cgpa[4]+sem_cgpa[5])/6;

             fprintf(quality,"\nCurrent_cgpa:<<<<<%f",current_cgpa[rt]);
